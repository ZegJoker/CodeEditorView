    ...
    public static func quote(_ argument: String) -> String {
        ...
        if argument.unicodeScalars.allSatisfy({ scalar in
            ...
            return (v >= 0x30 && v <= 0x39) // 0-9
                || (v >= 0x41 && v <= 0x5A) // A-Z
                || (v >= 0x61 && v <= 0x7A) // a-z
                || scalar == "_" || scalar == "-" || scalar == "." || scalar == "/" || scalar == "+" || scalar == "=" || scalar == "@" || scalar == "%"
        }) {
            return argument
        }
...
public final class ProcessHandle: @unchecked Sendable {
    ...
    private func emitStdout(_ data: Data) {
        lock.lock()
        if _terminated { lock.unlock(); return }
...
public struct ProcessService: Sendable {
    ...
    public func runCollecting(_ request: ProcessLaunchRequest) async throws -> (stdout: String, stderr: String, code: Int32) {
