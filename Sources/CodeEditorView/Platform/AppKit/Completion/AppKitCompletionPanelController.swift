    ...
    final class AppKitCompletionPanelController: NSObject, NSTableViewDataSource, NSTableViewDelegate {
        ...
        func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
            ...
            let cell =
                (tableView.makeView(withIdentifier: id, owner: self) as? CompletionCellView)
                ?? CompletionCellView()
            cell.identifier = id
            cell.configure(
                label: item.label, detail: item.detail, systemImage: item.systemImage, deprecated: item.deprecated)
            return cell
        }
