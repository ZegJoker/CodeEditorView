...
struct CSharpHighlightDebugTests {
    ...
    @Test func printRanges() async throws {
        ...
        for h in highlights.prefix(50) {
            ...
            print(
                "\(h.range.location)+\(h.range.length) cap=\(h.capture?.rawValue ?? "nil") raw=\(h.rawCapture ?? "") text=\(piece.debugDescription)"
            )
        }
        #expect(!highlights.isEmpty)
        // Keywords like "class" should be a single contiguous range, not 1-char fragments for the whole word only
        let classHits = highlights.filter {
            ($0.rawCapture ?? "").contains("keyword") && ns.substring(with: $0.range).contains("class")
                || ns.substring(with: $0.range) == "class"
        }
